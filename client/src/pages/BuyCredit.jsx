...
const BuyCredit = () => {
  return (
    <div className="min-h-[80vh] text-center pt-14 mb-10">
      <button className="border border-gray-400 px-10 py-3 rounded-full mb-6">
        {" "}
        Our Plans
      </button>
      ...
      <div className="flex flex-wrap justify-center gap-6 text-left">
        {plans.map((item, index) => (
          <div
            key={index}
            className="bg-white drop-shadow-sm rounded-lg py-13 px-8 text-gray-700 hover:scale-105 transition-all duration-300"
          >
            <img src={assets.logo_icon} width={40} alt="" />
            <p className="mt-3 font-semibold">{item.id}</p>
            <p className="test-sm">{item.desc}</p>
            <p className="mt-6">
